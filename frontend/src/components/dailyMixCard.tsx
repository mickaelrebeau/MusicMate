...
export function DailyMixCard({ playlistNumber, imageUrl}: { playlistNumber: number, imageUrl: string }) {
    return (
        <div className="w-[200px] shadow-2xl shadow-purple-500/30 rounded p-2 transition transform translate-y-3 hover:translate-y-0">
            ...
            <div className="p-2">
                <h3 className="text-lg font-medium">Daily Playlist {playlistNumber}</h3>
                <p className="text-sm text-slate-500">Name of some artist in the playlist</p>
            </div>
        </div>
    )
}